...
trait Flutterwave
{
    ...
    public function registerFlutterwaveEndpoints($endpoint_key = null)
    {
        ...
        if ($endpoint_key) {
            if (!array_key_exists($endpoint_key, $endpoints)) {
                throw new Exception("Endpoint key [$endpoint_key] not registered! Register it in registerFlutterwaveEndpoints() method");
            }
    ...
    {
        if (!$this->flutterwave_gateway_credentials) {
            $this->getFlutterwaveCredentials($output);
        }
