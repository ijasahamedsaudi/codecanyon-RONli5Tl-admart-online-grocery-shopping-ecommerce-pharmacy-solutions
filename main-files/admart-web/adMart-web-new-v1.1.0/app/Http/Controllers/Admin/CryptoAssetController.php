...
class CryptoAssetController extends Controller
{
    ...
    public function gatewayAssets(Request $request, $alias)
    {
        ...
        if (!$gateway->isCrypto() && !$this->isTatum($gateway)) {
            return abort(404);
        }
    ...
    public function walletBalanceUpdate(Request $request, $crypto_asset_id, $wallet_credentials_id)
    {
        ...
        if (count($wallet_info) == 0) {
            return back()->with(['error' => [__("Wallet Information Not Found!")]]);
        }
    ...
    public function walletStore(Request $request)
    {
        ...
        if ($validator->fails()) {
            return back()->withErrors($validator)->withInput()->with('modal', 'wallet-store');
        }
    ...
    public function walletTransactionSearch(Request $request, $crypto_asset_id, $wallet_credentials_id)
    {
        ...
        if (!$crypto_asset) {
            return Response::error([__('Crypto Asset Not Found!')]);
        }
        ...
        if (!$crypto_asset_address) {
            return Response::error([__('Crypto Address Not Found!')]);
        }
