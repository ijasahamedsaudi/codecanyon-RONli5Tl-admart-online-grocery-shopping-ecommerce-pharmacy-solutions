                        ...
                        <div class="account-wrapper otp-verification">
                            ...
                            <div class="verification-otp-content ptb-30">
                                ...
                                <p class="d-block text-center">{{ __('We sent a 6 digit code here') }}
                                    <strong>{{ $temp_data->identifier ?? '' }}</strong>
                                </p>
                                </p>
                            </div>
                            <form class="account-form" method="POST"
                                action="{{ setRoute('user.otp.login.verify.code', $temp_data->data->token) }}">
                                @csrf
...
@push('script')
    ...
    <script>
        var resendTime = "{{ 60 }}";
         var resendCodeLink = "{{ setRoute('user.otp.login.resend', $temp_data->data->token) }}";
