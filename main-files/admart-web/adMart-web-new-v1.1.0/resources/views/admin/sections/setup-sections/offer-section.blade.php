...
@section('content')
    <div class="custom-card">
        ...
        <div class="card-body">
                ...
                <div class="table-wrapper">
                    <div class="table-header justify-content-end">
                        <div class="table-btn-area">
                            <a href="#offer-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                                {{ __('Add Item') }}</a>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @forelse ($data->value->items ?? [] as $key => $item)
                                    <tr data-item="{{ json_encode($item) }}">
                                        ...
                                        <td> {{ $item->language->$system_default_lang->item_offer ?? "" }} </td>
            ...
            @include('admin.components.modals.site-section.offer.add')

            @include('admin.components.modals.site-section.offer.edit')
        </div>
    </div>
@endsection
...
@push('script')
    <script>
        openModalWhenError("offer-add", "#offer-add");
        openModalWhenError("offer-edit", "#offer-edit");
        ...
        $(".edit-modal-button").click(function() {
            ...
            editModal.find("input[name=" + default_language + "_item_name_edit]").val(oldData.language[
                default_language].item_name);
            editModal.find("input[name=" + default_language + "_item_offer_edit]").val(oldData.language[
                default_language].item_offer);
            editModal.find("input[name=price]").val(oldData.price);

            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_item_name_edit]").val((oldData.language[item
                    .code] == undefined) ? '' : oldData.language[item.code].item_name);
                editModal.find("input[name=" + item.code + "_item_offer_edit]").val((oldData.language[item
                    .code] == undefined) ? '' : oldData.language[item.code].item_offer);
            });

            fileHolderPreviewReInit("#offer-edit input[name=image_edit]");
            openModalBySelector("#offer-edit");
