    ...
    <div id="" class="table-area">
        <div class="modal-data">
            ...
            <div class="modal-form-data">
                <form class="modal-form" method="POST" action="{{ setRoute('admin.product.sub.categories.update') }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="target" value="{{ $sub_category->id }}">
                    <input type="hidden" name="old_image" value="{{ $sub_category->image }}">
                                        ...
                                        <div class="row">
                                            <div class="col-xl-12 col-lg-12 form-group">
                                                @include('admin.components.form.input', [
                                                    ...
                                                    'value' => old(
                                                        $lang_code . '_name',
                                                        $sub_category->data->language->$lang_code->name ?? ''),
                                                ])
                                            </div>
                                            <div class="col-xl-12 col-lg-12 form-group">
                                                @include('admin.components.form.input', [
                                                    ...
                                                    'value' => old(
                                                        $lang_code . '_title',
                                                        $sub_category->data->language->$lang_code->title ?? ''),
                                                ])
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <select id="" class="form--control select2-basic" name="category_id">
                                    ...
                                    @foreach ($category as $item)
                                        <option value="{{ $item->id }}"
                                            @if ($sub_category->category_id == $item->id) selected @endif>
