<div id="offer-edit" class="mfp-hide large">
                ...
                <input type="hidden" name="target" value="{{ old('target') }}">
                                ...
                                <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="modal-{{ $item->name }}" role="tabpanel" aria-labelledby="modal-{{$item->name}}-tab">
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            ...
                                            'value'     => old($lang_code . "_item_name_edit",$data->value->language->$lang_code->item_name ?? "")
                                        ])
                                    </div>
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            'label'     => __("Offer")."*",
                                            'name'      => $lang_code . "_item_offer_edit",
                                            'value'     => old($lang_code . "_item_offer_edit",$data->value->language->$lang_code->item_offer ?? "")
                                        ])
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="col-xl-12 col-lg-12 form-group">
                        @include('admin.components.form.input', [
                            'label' => __('price'),
                            'name' => 'price',
                            'value' =>old($data->value->price ??'') ,
                        ])
                    </div>
                    <div class="col-xl-12 col-lg-12 form-group">
                        @include('admin.components.form.input-file', [
                          'label' => __('Product Image') . '(400*400)',
                            'name' => 'image_edit',
                            'class' => 'file-holder',
