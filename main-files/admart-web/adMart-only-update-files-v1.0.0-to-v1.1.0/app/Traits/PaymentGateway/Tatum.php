...
trait Tatum
{
    ...
    public function tatumInit($output = null)
    {
        ...
        if (!$crypto_asset || !$crypto_active_wallet) {
            throw new Exception("Gateway is not available right now! Please contact with system administration");
        }
    ...
    public function tatumUserTransactionRequirements($trx_type = null)
    {
        ...
        if ($trx_type) {
            if (!array_key_exists($trx_type, $requirements)) {
                throw new Exception("User Transaction Requirements Not Found!");
            }
